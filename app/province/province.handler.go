...
// @Accept json
// @Produce json
// @Param req query request.PageProvince false "query string"
// @Success      200  {object}	response.Response
// @Failure      500  {object}  response.Response
// @Router /province/page [get]
func (h handler) Page(c echo.Context) error {
	var err error

	req := new(request.PageProvince)
...
// @Accept json
// @Produce json
// @Param req query request.ListProvince false "query string"
// @Success      200  {object}	response.Response
// @Failure      500  {object}  response.Response
// @Router /province/list [get]
func (h handler) List(c echo.Context) error {
	var err error

	req := new(request.ListProvince)
	if err = c.Bind(req); err != nil {
		return response.Error(http.StatusBadRequest, err.Error(), response.Payload{}).SendJSON(c)
	}
